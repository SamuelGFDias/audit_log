...
public sealed class SoftDeleteReflectionTests
{
    ...
    public async Task Should_cascade_soft_delete_to_children()
    {
        ...
        var notif = new Notificacao { Id = Guid.NewGuid(), Diagnostico = "Diagnóstico A", PacienteId = pacienteId, Paciente = paciente };
        ...
        var deletedPaciente = await db.Set<Paciente>().IgnoreQueryFilters().FirstAsync(x => x.Id == pacienteId);
        Assert.True(deletedPaciente.IsDeleted);
        Assert.NotNull(deletedPaciente.DeletedAt);

        var deletedNotif = await db.Set<Notificacao>().IgnoreQueryFilters().FirstAsync(x => x.Id == notif.Id);
    ...
    public async Task Should_throw_RestrictDeleteViolationException_when_dependents_exist()
    {
        ...
        var pacienteId = Guid.NewGuid();
        var paciente = new Paciente { Id = pacienteId, Nome = "Restrict Test", Endereco = new PacienteEndereco { Logradouro = "Rua", Cidade = "Cid" } };
        var documento = new PacienteDocumento { Id = Guid.NewGuid(), NomeDocumento = "Doc.pdf", PacienteId = pacienteId };
    ...
    public async Task Should_set_null_on_referencing_FK_when_SetNull()
    {
        ...
        var pacienteId = Guid.NewGuid();
        var paciente = new Paciente { Id = pacienteId, Nome = "SetNull Test", Endereco = new PacienteEndereco { Logradouro = "Rua", Cidade = "Cid" } };
        var notif = new Notificacao { Id = Guid.NewGuid(), Diagnostico = "Teste", PacienteId = pacienteId };
        paciente.Notificacoes.Add(notif);
        db.Set<Paciente>().Add(paciente);
        await db.SaveChangesAsync();
        ...
        var updatedNotif = await db.Set<Notificacao>().IgnoreQueryFilters().FirstAsync(x => x.Id == notif.Id);
    ...
    public async Task Should_filter_out_soft_deleted_entities_by_default()
    {
        ...
        var paciente1 = new Paciente { Id = Guid.NewGuid(), Nome = "Ativo", Endereco = new PacienteEndereco { Logradouro = "Rua", Cidade = "Cid" } };
        var paciente2 = new Paciente { Id = Guid.NewGuid(), Nome = "Deletado", Endereco = new PacienteEndereco { Logradouro = "Rua2", Cidade = "Cid2" } };
    ...
    public async Task Should_return_all_entities_with_IgnoreQueryFilters()
    {
        ...
        var todos = await db.Set<Paciente>().IgnoreQueryFilters().ToListAsync();
    ...
    public async Task Should_not_throw_when_deleting_entity_without_dependents_Restrict()
    {
        ...
        var paciente = new Paciente { Id = Guid.NewGuid(), Nome = "Sem Dependentes", Endereco = new PacienteEndereco { Logradouro = "Rua", Cidade = "Cid" } };
        db.Set<Paciente>().Add(paciente);
        await db.SaveChangesAsync();
    ...
    public async Task Should_convert_physical_delete_to_soft_delete_for_ISoftDeleteEntity()
    {
        ...
        var paciente = new Paciente { Id = Guid.NewGuid(), Nome = "Físico", Endereco = new PacienteEndereco { Logradouro = "Rua", Cidade = "Cid" } };
        db.Set<Paciente>().Add(paciente);
        await db.SaveChangesAsync();
